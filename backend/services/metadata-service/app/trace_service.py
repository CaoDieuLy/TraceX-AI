"""
Trace pipeline — 6-stage person trajectory reconstruction.

Stage 1: Seed tracklet + 24h window
Stage 2: Dynamic Topology Pruning (reachable cameras per time slot)
Stage 3: Spatiotemporal Retrieval (DB query by camera + time + embedding sim)
Stage 4: Trajectory / Path Search & Re-ranking
Stage 5: Final Trajectory + Evidence Clips
Stage 6: Human-in-the-loop feedback (gallery update + query refinement)

Latency target: 5–30s for stages 1-5.
"""
...
from .models import PersonCandidate, QueueVideoAsset
from .service import candidate_to_ranking_payload, _queue_video_map

logger = logging.getLogger(__name__)

_TOPOLOGY_PATH = Path(__file__).parent.parent.parent.parent.parent / "config" / "camera_topology.json"
...
class CameraTopologyGraph:
    """Bidirectional camera adjacency graph loaded from camera_topology.json."""
    ...
    def reachable_cameras(
        ...
        """
        Return [(camera_id, min_arrival_sec, max_arrival_sec)] reachable
        from `from_camera` given the person left at `after_seconds`.
        """
    ...
    def travel_plausibility(
        ...
        """
        Score ∈ [0, 1] for whether elapsed_seconds is plausible for travel
        from from_camera to to_camera. Returns 0 if impossible, 1 if ideal.
        """
        for edge in self.neighbors(from_camera):
            if edge.to_camera == to_camera:
                ...
                if elapsed_seconds <= edge.max_seconds:
                    return 1.0  # within expected window
                # Beyond max but same floor/zone: decay smoothly
                decay = math.exp(-(elapsed_seconds - edge.max_seconds) / 120.0)
                return max(0.1, decay)
        # Unknown direct edge — use default cap
        unknown_max = float(self._defaults.get("unknown_camera_max_seconds", 120))
        if elapsed_seconds > unknown_max:
            return 0.0
        return 0.3  # possible but not confirmed by topology
...
def build_seed(
    ...
    # Resolve recorded_start from video asset
    ...
    # Tracklet time offsets from timeline
...
def pruned_camera_set(
    ...
    """
    BFS over topology graph up to max_hops from seed camera.
    Returns the set of cameras that are reachable (candidates worth searching).
    """
    if not seed.camera_id:
        return set(all_cameras)  # no topology info, search all
    ...
    # Include cameras not in topology (unknown layout) with a flag
...
def spatiotemporal_retrieval(
    ...
    """
    Query DB for candidates in camera_scope within seed's time window,
    filter by appearance embedding similarity ≥ min_similarity.
    """
...
def build_trajectory(
    ...
    """
    Greedy forward+backward path construction from seed.
    Splits candidates into before/after seed, builds chronological chain.
    """
    # Seed anchor time
    ...
    def chain(
        ...
        for cand in ordered:
            ...
            # Velocity: assume 5m/s max sprint, reject physically impossible cross-floor jumps
...
def rerank_trajectory(segments: list[TrajectorySegment]) -> list[TrajectorySegment]:
    """
    Remove segments that violate temporal ordering or are physically impossible.
    Apply cross-camera deduplication (same person shouldn't appear in 2 cams simultaneously).
    """
    if not segments:
        return []
    ...
    for seg in segments:
        # Drop if starts before previous segment ended (temporal overlap = duplicate)
        if prev_end and seg.abs_start and seg.abs_start < prev_end:
            continue
        # Drop topology-impossible hops
...
class FeedbackPayload:
    candidate_id: str
    confirmed_segment_ids: list[str]   # candidate_ids user confirmed as same person
    rejected_segment_ids: list[str]    # candidate_ids user rejected
...
def apply_feedback(
    ...
    """
    Gallery update + query refinement after user confirms/rejects segments.

    1. Re-build seed embedding by averaging confirmed segments' embeddings
    2. Expand window by 20% to catch edges
    3. Re-run trace with richer seed
    """
    ...
    # Average embeddings from confirmed segments — richer gallery representation
...
def run_trace(session: Session, seed: SeedInfo) -> TraceResult:
    """Run stages 2-5 with a pre-built seed."""
    # Stage 2: topology pruning
    all_cameras = [
...
def trace_from_candidate_id(
    ...
    """Full pipeline entry: stage 1 → stage 5."""
