...
function HomeViewInner() {
  ...
  useEffect(() => { setCurrentUser(loadSessionUser()); }, []);
  ...
  useEffect(() => {
    ...
    void getSearchHistory()
      .then((items) => { if (cancelled) return; setHistoryItems(items); })
      .catch((err) => { if (cancelled) return; showToast(err instanceof Error ? err.message : "Không thể tải lịch sử.", "error"); })
      .finally(() => { if (cancelled) return; setHistoryLoading(false); });
    ...
    return (
      <div className="mx-auto w-full max-w-4xl">
        <div className="mb-8 text-center">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700 dark:text-blue-300">Search Intelligence</p>
        ...
        <ul className="space-y-3">
          {historyItems.map((row) => (
            <li
              key={row.queryId}
              className="rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)] transition hover:border-blue-200 hover:shadow-[0_16px_30px_rgba(37,99,235,0.1)] dark:border-slate-800 dark:bg-slate-900 dark:hover:border-blue-700"
            >
              <button
                type="button"
                ...
                onClick={() => {
                  ...
                  if (!row.videoId) { setSelectedHistoryClips([]); return; }
        ...
        {selectedHistoryClips.length > 0 ? (
          <div className="mt-8"><VideoList clips={selectedHistoryClips} /></div>
        ) : null}
      </div>
    );
  }
  ...
  if (!hasSearched) {
    return (
      <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col items-center pb-24 md:pb-0">
        ...
        {/* Admin import panel */}
        {isAdmin ? (
          <div className="mb-6 flex w-full items-center justify-between gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3 dark:border-emerald-500/30 dark:bg-emerald-500/10">
            <div>
              <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-200">Import video mới</p>
            ...
            <button
              ...
              className={[
                ...
                isImporting
                  ? "cursor-not-allowed border-emerald-200 bg-emerald-100 text-emerald-400 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-500"
                  : "border-emerald-300 bg-white text-emerald-700 hover:bg-emerald-100 dark:border-emerald-500/50 dark:bg-transparent dark:text-emerald-300 dark:hover:bg-emerald-500/10",
              ].join(" ")}
            >
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-black uppercase tracking-wide text-blue-700 shadow-[0_8px_18px_rgba(37,99,235,0.12)] dark:border-blue-500/40 dark:bg-blue-500/10 dark:text-blue-300">
        ...
        <section className="mt-8 w-full text-center">
          ...
          <p className="mx-auto mt-7 max-w-3xl text-lg leading-8 text-slate-600 dark:text-slate-300">
            Tập trung mô tả ngắn gọn và chính xác để hệ thống phân tích nhanh hơn, đối chiếu đặc điểm hiệu quả hơn và
        ...
        <section className="mt-16 grid w-full grid-cols-1 gap-6 md:grid-cols-3">
          {guideCards.map((card, index) => (
            ...
            >
              <span className="pointer-events-none absolute right-5 top-1 text-6xl font-black leading-none text-slate-50 dark:text-slate-800/70">
        ...
        <section className="mt-14 grid w-full grid-cols-1 gap-4 md:grid-cols-3">
          {HOME_GUIDE_FEATURES.map((feature, index) => (
            <article
              key={feature.title}
              className={[
                "flex min-h-[116px] items-center gap-4 rounded-2xl border bg-white p-5 text-left shadow-[0_12px_28px_rgba(15,23,42,0.06)] transition-colors dark:bg-slate-900",
                index === 0
                  ? "border-blue-300 shadow-[0_14px_30px_rgba(37,99,235,0.14)] dark:border-blue-500/50"
        ...
        <section className="mt-12 w-full rounded-3xl border border-slate-200 bg-white p-6 text-left shadow-[0_14px_34px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-slate-900 md:p-8">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {HOME_GUIDE_SUPPORTED_CRITERIA.map((criterion) => (
              <span
                key={criterion}
                className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-800 shadow-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
              >
                {criterion}
              </span>
            ))}
        ...
        <section className="mt-6 w-full rounded-3xl border border-amber-200 bg-amber-50/70 p-6 text-left shadow-[0_14px_34px_rgba(146,64,14,0.08)] dark:border-amber-500/30 dark:bg-amber-500/10 md:p-8">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {HOME_GUIDE_UNSUPPORTED_EXAMPLES.map((example) => (
              <p
                key={example}
                className="rounded-2xl border border-amber-200 bg-white px-4 py-3 text-sm font-semibold leading-6 text-slate-700 dark:border-amber-500/30 dark:bg-slate-950 dark:text-slate-200"
              >
                {example}
              </p>
            ))}
  ...
  return (
    <div className="flex flex-col gap-8">
      {isLoading ? (
        <p className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-900 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-200">
          Đang tải kết quả...
        </p>
      ) : null}
      {!isLoading && !error && results.length === 0 ? (
        <p className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          Không tìm thấy kết quả phù hợp.
        </p>
      ) : null}
      <VideoGrid items={pageItems} />

      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_10px_28px_rgba(15,23,42,0.06)] dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <button
            ...
            onClick={async () => {
              if (!isLastLoadedPage) { setGridPage(gridPage + 1); return; }
              if (hasMore) { const added = await loadMore(); if (added) setGridPage(gridPage + 1); }
            }}
            className="rounded-xl bg-slate-950 px-5 py-2.5 text-sm font-bold text-white shadow-[0_12px_28px_rgba(15,23,42,0.22)] transition duration-200 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-blue-600 dark:hover:bg-blue-500"
          >
            Tiếp theo
          </button>
        </div>
