...
export function MainShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 transition-colors dark:bg-slate-950">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-y-auto bg-slate-50 transition-colors dark:bg-slate-950">
        <header
          className={
            hasSearched
              ? "sticky top-0 z-30 border-b border-slate-200/70 bg-white/90 px-6 pb-4 pt-5 backdrop-blur-xl transition-colors dark:border-slate-800 dark:bg-slate-950/90"
              : "px-6 pb-2 pt-5"
          }
        >
          <div className="mx-auto flex max-w-6xl flex-col gap-4">
            <div className="rounded-2xl border border-slate-200/85 bg-white/65 p-3 shadow-[0_12px_30px_rgba(15,23,42,0.07)] backdrop-blur-md transition-colors dark:border-slate-800 dark:bg-slate-950/80">
